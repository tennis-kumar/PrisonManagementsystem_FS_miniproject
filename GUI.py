...
def login_check():
	...
	if pos == -1:
		...
		if(verify_password(word[1], password)):
			...
			tkinter.messagebox.showinfo("Login"," Password that you have entered is incorrect.Please re enter")
			return(login_in)
		f2.close()
...
def Admin_Opt():
		...
		addbutton=Button(opt_menu,command=add_book,text=" Add Prisoner ",bg='light pink',height=1,width=12,font=k_font)
		delbutton=Button(opt_menu,command=del_book,text=" Remove Prisoner ",bg='light blue',height=1,width=12,font=k_font)
...
def add_book():
	...
	book_id_label=Label(add_menu,text="Prisoner ID (Should be 5 digit)")
	book_label=Label(add_menu,text="Prisoner Name")
	author_label=Label(add_menu,text="Crime Committed")
	book_id=Entry(add_menu)
	book_name=Entry(add_menu)
	author_name=Entry(add_menu)
	addbutton1=Button(add_menu,command=add_check,text=" Add Prisoner ",bg='dark orange',height=1,width=10,font=k_font)
...
def add_check():
	...
	if len(b_name)==0:
		tkinter.messagebox.showinfo("Add Book","You did not type a Prisoner name O_O")
		add_menu.lift()
		return(add_book)
	...
	if pos != -1:
		tkinter.messagebox.showinfo("Book","Prisoner already present.Please try again")
		add_menu.lift()
		return(add_book)
	...
	f22.close()
	key_sort('Bindex.txt')
	tkinter.messagebox.showinfo("Add","Prisoner added Successfully!")
	add_menu.destroy()
...
def del_book():
	...
	b_label=Label(del_menu,text="Prisoner ID")
	rb_id=Entry(del_menu)
	delbutton1=Button(del_menu,command=del_check,text=" Remove ",bg='dark orange',height=1,width=10,font=k_font)
	...
	borrow_label3=Label(del_menu,text="Crime Committed")
...
def del_check():
	...
	if(pos == -1):
		tkinter.messagebox.showinfo("Delete","Prisoner not present.Please reenter")
		del_menu.destroy()
		return(del_book)
	else:
		f = open ('BData.txt', 'r')
		...
		w1 = l1.split('|')
		if(w1[3] == 'N'):
			tkinter.messagebox.showinfo("Delete","Prisoner currently on job. Please try another prisoner")
			del_menu.destroy()
			return(del_book)
	...
	with open("Bindex.txt",'r+') as file:
		...
		while line:
			...
			if words[0]==del_id:
				...
				index=file.tell()
				line=file.readline()
	tkinter.messagebox.showinfo("Delete","Prisoner Successfully removed")
	del_menu.destroy()
...
def Main_Menu():
	...
	orionLabel=Label(base, text="TV CENTRAL PRISON",bg='dark orange',font=("Castellar", "50","bold","italic","underline"),fg="black")
	...
	#Buttons
	borrow_but=Button(bottomFrame,bg="black",fg="white",text="Add to Job",font=in_font,height=5,width=15,command=borrow_in)
	return_but=Button(bottomFrame,bg="dark orange",text="Remove from Job",font=in_font,height=5,width=15,command=return_in)
	search_but=Button(bottomFrame,bg="black",fg="white",text="Search Prisoners",font=in_font,height=5,width=15,command=search_in)
...
def borrow_in():
	...
	borrow_label1=Label(borrow_menu,text="<<< Please enter the PRISONER ID that you wish to add >>>",font=("Times", "20","bold","italic"),bg="light blue")
	borrow_label=Label(borrow_menu,text="Id")
	borrow_label2=Label(borrow_menu,text="Prisoner Name")
	borrow_label3=Label(borrow_menu,text="Crime")
	borrow_label4=Label(borrow_menu,text="Availability")

	borrow_entry1=Entry(borrow_menu,width=50)
	borrow_button1=Button(borrow_menu,text="ADD",command=borrow_check,font=("Times new roman","10","bold"),bg="light blue")
...
def borrow_check():
	...
	for l in f:
		...
		if l[0] ==  id:
			count += 1
	if count >= 3:
		tkinter.messagebox.showinfo("Borrow", "Cannot take more than 3 prisoners")
		borrow_menu.destroy()
		...
		if pos == -1:
			tkinter.messagebox.showinfo("Borrow","The Prisoner ID that you had entered is not in our database,sorry,please enter a different Prisoner ID")
			borrow_menu.lift()
		else:
			f2 = open('BData.txt', 'r+')
			f2.seek(pos)
			...
			if(w2[3] == 'Y'):
				...
				f2.write(l3)
				f2.close()
				tkinter.messagebox.showinfo("Borrow","Prisoner succesfully added to Job. valid till:" +'\n'+ str(enddate) )
				...
				f3.close()
				key_sort('Record.txt')
				Done2=tkinter.messagebox.askyesno("Borrow","Do you want to add another prisoner?")
				if Done2==True:
					borrow_menu.destroy()
					borrow_in()
				else:
				...
				tkinter.messagebox.showinfo("Borrow","This Prisoner is currently unavailable, please select another Prisoner")
				borrow_menu.lift()
...
def return_in():
	...
	return_label1=Label(return_menu,text=" Please enter the PRISONER ID that you wish to return ",font=("Times", "12","bold","italic"),bg="light blue")
...
def return_check():
	...
	if(bbook in record_verification):
		...
		if pos != -1:
			...
			w1 = l1.split('|')
			if(w1[3] == 'N'):
				tkinter.messagebox.showinfo("Return","Prisoner successfully removed from current job on"+'\n'+str(date))
				f1.seek(pos)
				...
				for l2 in lines:
					...
					if l3[1] == bbook and l3[0] == id:
						...
						f3.write(l2)
				f3.close()
				Done3=tkinter.messagebox.askyesno("Return","Do you want to remove another prisoner?")
				if Done3==True:
					f1.close()
					return_menu.destroy()
					return_in()
				...
				tkinter.messagebox.showinfo("This Prsoner has been removed, please select another prisoner")
			f1.close()
	else:
		tkinter.messagebox.showinfo("Return","The prisoner that you had entered is invalid.Please reenter a different Prisoner")
		return_menu.lift()
...
def search_in():
	...
	search_label1=Label(search_menu,text="Search through our database to check if your desired prisoner is available",font=("Times", "12","bold","italic"),bg="light blue")
...
def search_check():
	...
	if (pos == -1):
		tkinter.messagebox.showinfo("Search","Sorry,this Prisoner does not exist in our database")
	else:
		search_menu2=Tk()
		...
		search_result.insert(3,"Crime:" + author)
